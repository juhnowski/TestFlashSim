...
void load_config(void) {
	...
	// ИСПРАВЛЕНИЕ: Объявляем массив ровно один раз с фиксированным размером
	char absolute_path[4096];

	if (realpath(config_name, absolute_path) != NULL) {
		printf("[КОНФИГУРАЦИЯ]: Успешно загружен файл: %s\n", absolute_path);
	} else {
		printf("[КОНФИГУРАЦИЯ]: Успешно загружен файл: %s\n", config_name);
	}
	fflush(stdout);
...
void print_config(FILE *stream) {
	...
	// Добавление параметров ZNS в зависимости от режима FTL
	fprintf(stream, "\n=== ПАРАМЕТРЫ ZNS СТРУКТУРЫ ===\n");
	if (FTL_IMPLEMENTATION == 5) {
		unsigned int zns_block_size = (BLOCK_SIZE > 0) ? BLOCK_SIZE : 64;
		fprintf(stream, "ZNS_ENABLED: 1 (FTL_IMPLEMENTATION == 5)\n");
		fprintf(stream, "ZNS_TOTAL_ZONES: 4096\n");
		fprintf(stream, "ZNS_ZONE_SIZE (PAGES): %u\n", zns_block_size);
		fprintf(stream, "ZNS_TOTAL_CAPACITY (PAGES): %lu\n", 4096 * (unsigned long)zns_block_size);
	} else {
		fprintf(stream, "ZNS_ENABLED: 0 (Классический режим эмуляции FTL)\n");
	}
	fprintf(stream, "================================\n\n");
