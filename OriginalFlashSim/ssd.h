...
struct ZnsZone {
    ulong wptr = 0;
    bool is_full = false;
    ulong erase_count = 0; // Добавляем счетчик стираний для Теста 7
};
...
public:
	...
	enum status execute_hardware_step(Event *cur);
	...
	friend class Block_manager;
	friend class FtlImpl_Zns;

	Stats stats;
	void print_ftl_statistics();
	const FtlParent &get_ftl(void) const;
	enum status direct_zns_gate(unsigned long lba, int is_write);
    enum status reset_zns_zone(uint zone_id);
    void flush_smart_to_eeprom();
    void set_temperature(double temp);
    bool check_thermal_shutdown();
private:
	...
	Ssd &ssd;
	FtlParent *ftl;
    static ZnsZone* zns_zones;
    static ulong total_zns_zones;
    bool validate_and_advance_zns(Event &event);
    ulong open_zones_count_check();
    void simulate_crypto_write(unsigned long lba, ulong zone_id, uint target_page);
    bool simulate_crypto_read(unsigned long lba, ulong zone_id, uint target_page, uint wptr);
    void simulate_crypto_erase(ulong zone_id);
...
class FtlImpl_Zns : public FtlParent {
public:
    FtlImpl_Zns(Controller &controller);
    virtual ~FtlImpl_Zns(void);
    enum status read(Event &event);
    enum status write(Event &event);
    enum status trim(Event &event);
};
